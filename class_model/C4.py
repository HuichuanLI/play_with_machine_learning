# -*- coding:utf-8 -*-
# @Time : 2022/2/12 10:50 下午
# @Author : huichuan LI
# @File : C4.py
# @Software: PyCharm
# -*- coding:utf-8 -*-
# @Time : 2022/2/12 5:25 下午
# @Author : huichuan LI
# @File : CART.py
# @Software: PyCharm
...
### entropy熵
def calculate_entropy(ele):
    ele = ele.tolist()
    probs = [ele.count(i) / len(ele) for i in np.unique(ele)]
...
class ClassificationTree(BinaryDecisionTree):
    ...
    def _calculate_entropy(self, y, y1, y2):
        p = len(y1) / len(y)
        entropy_orignal = calculate_entropy(y)
        entropy_gain = p * calculate_entropy(y1) + (1 - p) * calculate_entropy(y2)
        if entropy_orignal != 0:
            entropy_gain_percentage = (entropy_orignal - entropy_gain) / entropy_orignal
        else:
            entropy_gain_percentage = (entropy_orignal - entropy_gain)
        return entropy_gain_percentage
    ...
    # 分类树拟合
    def fit(self, X, y):
        self.impurity_calculation = self._calculate_entropy
